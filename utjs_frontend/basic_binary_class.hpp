...
public:
    ...
    friend std::istream& operator >>(std::istream& fin,Bint& x){
        ...
        friend std::ostream& operator <<(std::ostream& fout,const Bint& x){
            fout.write(reinterpret_cast<const char*>(&x.data),sizeof(int));
            return fout;
        }
...
public:
    ...
    friend std::ostream& operator <<(std::ostream& fout, const Bchar& x){
