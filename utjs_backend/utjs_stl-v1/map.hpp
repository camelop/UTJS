...
public:
	...
	friend std::istream& operator >>(std::istream &fin, map& m) {
		m.clear();
		m.head = new map_node;
	...
	void read_node(std::istream &fin, map_node* &x) {
		if (x->data == NULL) x->data = new value_type;
		fin >> (*(x->data));
		char q[2];
		fin.read(q, 2* sizeof(char));
		if (q[0]) {
			x->l = new map_node;
			read_node(fin, x->l);
		}
	...
	friend std::ostream& operator <<(std::ostream& fout, map& m) {
		m.write_node(fout, m.head);
		return fout;
	}
	void write_node(std::ostream &fout, map_node* x) {
		fout << *(x->data);
		char q[2];
		q[0] = 0; q[1] = 1;
		if (x->l != NULL) fout.write(q+1,sizeof(char)); else fout.write(q, sizeof(char));
		if (x->r != NULL) fout.write(q+1, sizeof(char)); else fout.write(q, sizeof(char));
